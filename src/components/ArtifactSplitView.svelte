<script lang="ts">
  ...
  import { getActiveArtifact, nextArtifact, prevArtifact } from '../lib/artifact-state';
  ...
  let { queue, isMobile, activeEntry, onclose, onFixArtifact, artifactError }: Props = $props();
...
{#if activeEntry}
  ...
  {:else}
    ...
    <div class="hidden md:flex flex-col w-[50%] min-w-[360px] bg-[#faf7f2] overflow-hidden shadow-2xl z-10" style="resize: horizontal;">
      <div class="flex items-center justify-between px-4 py-3 bg-stone-800 text-white shrink-0">
        <div class="flex items-center gap-3 min-w-0">
          {#if msgEntries.length > 1}
            <div class="flex items-center gap-1">
              <button onclick={goPrev} class="text-[11px] px-1.5 py-0.5 rounded text-stone-400 hover:text-white transition-colors" aria-label="Previous artifact">◂</button>
              <span class="text-[10px] text-stone-400 font-medium tabular-nums">{activeIdx + 1}/{msgEntries.length}</span>
              <button onclick={goNext} class="text-[11px] px-1.5 py-0.5 rounded text-stone-400 hover:text-white transition-colors" aria-label="Next artifact">▸</button>
            </div>
        ...
        <div class="flex items-center gap-1">
          <button onclick={() => splitTab = 'code'} class="text-[10px] px-2.5 py-1 rounded-md {splitTab === 'code' ? 'bg-white/20 text-white font-bold' : 'text-stone-400 hover:text-white'}">Code</button>
          <button onclick={() => splitTab = 'preview'} class="text-[10px] px-2.5 py-1 rounded-md {splitTab === 'preview' ? 'bg-white/20 text-white font-bold' : 'text-stone-400 hover:text-white'}">Preview</button>
      ...
      <div class="flex-1 overflow-auto bg-[#faf7f2]">
        {#if artifactError}
          <div class="bg-red-50 border-b border-red-200 px-4 py-2 flex items-center justify-between gap-2">
            <div class="text-xs text-red-700 truncate min-w-0"><span class="font-bold">Error:</span> {artifactError}</div>
            <button onclick={() => currentEntry && onFixArtifact(currentEntry.artifact.code, artifactError)} class="text-[10px] px-3 py-1 bg-red-600 hover:bg-red-700 text-white rounded-md font-bold shrink-0 transition-all">Fix with AI</button>
          </div>
        {/if}
        {#if currentEntry}
  ...
  {#if chipBases.length > 0 && !isMobile}
    <div class="hidden md:block absolute bottom-4 right-4 max-w-xs max-h-64 overflow-y-auto space-y-1 p-2 bg-white/90 backdrop-blur rounded-xl border border-stone-200 shadow-lg z-20">
      <div class="text-[10px] font-semibold text-stone-400 px-2 py-1">Other artifacts</div>
      {#each chipBases as entry}
        <ChatArtifactChip entry={entry} onclick={() => {}} />
      {/each}
    </div>
  {/if}
{/if}
