...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    if (!body?.type || !body?.code) {
      return apiError({ requestId, status: 400, code: 'INVALID_REQUEST', message: 'Artifact type and code are required.', headers: RESPONSE_HEADERS });
    }

    const type = String(body.type || '').toLowerCase();
    const code = String(body.code || '');
    if (code.length > 200_000) {
      return apiError({ requestId, status: 413, code: 'ARTIFACT_TOO_LARGE', message: 'Artifact source is too large to render safely.', headers: RESPONSE_HEADERS });
    }
    if (!KROKI_RENDERABLE.has(type)) {
      return apiError({ requestId, status: 400, code: 'UNSUPPORTED_ARTIFACT_TYPE', message: `${type} is not server-renderable.`, headers: RESPONSE_HEADERS });
    }
