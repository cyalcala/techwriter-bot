export function getDiagramRenderDoc(type: string, code: string): string {
  ...
  const base = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=5,user-scalable=yes"><style>*{margin:0;padding:0;box-sizing:border-box}body{background:#faf7f2;padding:16px;overflow:auto;-webkit-overflow-scrolling:touch;font-family:system-ui,sans-serif;min-height:100dvh}.error{color:#b91c1c;background:#fef2f2;padding:12px;border-radius:8px;font-size:13px;margin:8px 0}svg{height:auto;display:block;margin:16px auto;min-width:300px}</style>`;

  switch (type) {
    case 'mermaid':
      return `${base}<script>var c=document.getElementById('d');if(c){c.textContent=\`${escaped}\`}<\/script><script src="https://cdn.jsdelivr.net/npm/mermaid@10/dist/mermaid.min.js"><\/script><script>mermaid.initialize({startOnLoad:true,theme:'neutral',securityLevel:'loose',flowchart:{htmlLabels:true}});<\/script></head><body><pre class="mermaid" id="d">${escaped}</pre><script>setTimeout(()=>{const s=document.querySelector('svg');if(!s){document.body.innerHTML+='<div class=error>Mermaid could not render this diagram. The syntax may have issues.</div>'}else{s.style.width='auto';s.style.maxWidth='none'}},1000)<\/script></body></html>`;
